  ...
  return (
    <article className="bg-white/90 border border-emerald-100 rounded-xl shadow-sm p-4">
      <header className="mb-3 flex items-center justify-between">
        ...
        <span className="text-xs text-slate-500">Objetivo: {serie.objetivo ?? '—'}</span>
      </header>
        ...
        <ul className="space-y-2">
          {serie.puntos.map((p) => (
            <li key={p.label} className="flex justify-between">
              ...
              <strong className="text-slate-900">{formatValor(p.valor, serie.unidad)}</strong>
            </li>
          ))}
        </ul>
      </div>

      <footer className="mt-4 text-xs text-slate-500">Gráficos interactivos se pueden añadir después.</footer>
    </article>
  );
};
