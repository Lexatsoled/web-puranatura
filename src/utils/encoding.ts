 ...
 * Entradas: text (string) - Texto potencialmente corrupto (ej: "ángel")
 ...
 *
 * Ejemplo de uso:
 *   normalizeText("¿Cómo estás?") → "¿Cómo estás?"
 *   normalizeText("España") → "España"
 *
 * Casos especiales:
 ...
 * Entradas: text (string) - Texto a analizar (ej: "Hola ángel")
 ...
 *
 * Ejemplo de uso:
 *   const userInput = "¿Cómo estás?"; // Input corrupto del usuario
 ...
 *
 * Casos que maneja:
 *   - Strings directos: "ángel" → "ángel"
 *   - Arrays de strings: ["té", "café"] → ["té", "café"]
 *   - Objetos anidados: { user: { name: "ángel" } } → recursivo
 *   - Arrays de objetos: [{ name: "ángel" }, ...] → recursivo
    ...
    validateField: (value: string): { valid: boolean; message?: string } => {
      if (hasMojibake(value)) {
        return {
          valid: false,
          message:
            'El texto contiene caracteres corruptos. Por favor, revisa el contenido.',
        };
      }
      return { valid: true };
    },
