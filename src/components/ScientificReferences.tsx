...
const ScientificReferences: React.FC<ScientificReferencesProps> = ({ 
  ...
  if (!product.scientificReferences || product.scientificReferences.length === 0) {
    return (
      <div className={`text-center py-8 ${className}`}>
        <div className="text-gray-500">
          ...
          <p className="text-sm mt-2">Las referencias científicas para este producto están siendo recopiladas y serán añadidas pronto.</p>
        </div>
      </div>
    );
  }
        ...
        {product.scientificReferences.map((ref, index) => (
          <div
            ...
            className={`border-2 rounded-lg transition-all duration-200 ${
              RelevanceColors[ref.relevance]
            } ${
              expandedReference === index ? 'shadow-lg' : 'shadow-sm hover:shadow-md'
            }`}
          >
            <div
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <div className="flex flex-wrap items-center gap-2 mb-2">
                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${StudyTypeColors[ref.studyType]}`}>
                    ...
                    <span className={`px-2 py-1 rounded text-xs font-medium ${
                      ref.relevance === 'alta' ? 'bg-green-100 text-green-700' :
                      ref.relevance === 'media' ? 'bg-yellow-100 text-yellow-700' :
            ...
            {expandedReference === index && (
              <div className="border-t border-gray-200 p-4 bg-white bg-opacity-50">
                <div className="space-y-4">
                  <div>
                    <h5 className="font-semibold text-gray-800 mb-2">Hallazgos Clave:</h5>
                    <ul className="space-y-1">
                      {ref.keyFindings.map((finding, findingIndex) => (
                        <li key={findingIndex} className="text-sm text-gray-700 flex items-start">
      ...
      <div className="mt-6 p-4 bg-blue-50 rounded-lg border-l-4 border-blue-400">
        <div className="flex items-start">
          ...
          <div className="text-sm">
            <p className="text-blue-800 font-medium mb-1">Interpretación de Estudios</p>
            <p className="text-blue-700">
              Las referencias científicas se presentan con fines informativos. Los resultados pueden variar entre individuos. 
              Siempre consulte con un profesional de la salud antes de usar este o cualquier suplemento.
            </p>
          </div>
        </div>
      </div>
