...
const AuthModal: React.FC<AuthModalProps> = ({
  ...
  const validateForm = () => {
    ...
    if (!formData.email) {
      ...
      newErrors.email = 'El email no es vï¿½lido';
    }

    if (!formData.password) {
      newErrors.password = 'La contraseï¿½a es requerida';
    } else if (formData.password.length < 6) {
      newErrors.password = 'La contraseï¿½a debe tener al menos 6 caracteres';
    }
    ...
    if (activeTab === 'register') {
      ...
      if (!formData.confirmPassword) {
        newErrors.confirmPassword = 'Confirma tu contraseï¿½a';
      } else if (formData.password !== formData.confirmPassword) {
        newErrors.confirmPassword = 'Las contraseï¿½as no coinciden';
      }
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (activeTab === 'login') {
        ...
        if (!success) {
          setErrors({ general: 'Email o contraseï¿½a incorrectos' });
        }
      } else {
        ...
        if (!success) {
          setErrors({ general: 'El email ya estï¿½ registrado' });
        }
      }
      ...
      setErrors({ general: 'Ha ocurrido un error. Intï¿½ntalo de nuevo.' });
    } finally {
          ...
          >
            ...
            <div className="p-6 border-b border-gray-200">
              <div className="flex items-center justify-between">
                ...
                >
                  {activeTab === 'login' ? 'Iniciar Sesiï¿½n' : 'Crear Cuenta'}
                </h2>
                <button
                  onClick={onClose}
                  className="text-gray-400 hover:text-gray-600 transition-colors"
                  aria-label="Cerrar modal de autenticaciï¿½n"
                >
                  <svg
                    className="w-6 h-6"
                    fill="none"
                  ...
                  <div>
                    ...
                    <div className="mt-2 flex items-center">
                      ...
                      <label
                        ...
                        className="ml-2 block text-sm text-gray-700"
                      >
                        Mostrar contraseï¿½a de confirmaciï¿½n
                      </label>
                    </div>
                    {errors.confirmPassword && (
                      <p className="text-red-600 text-xs mt-1">
