...
const RelatedProducts: React.FC<RelatedProductsProps> = ({
  ...
  const isEnergySystemProduct = energySystemProducts.includes(currentProduct.id);
  const isAntiAgingSystemProduct = antiAgingSystemProducts.includes(currentProduct.id);
  const isArticularSystemProduct = articularSystemProducts.includes(currentProduct.id);
    ...
    relatedProducts = products
      .filter(
        (product) => {
          ...
          const hasCommonCategories = currentProduct.categories && product.categories &&
            currentProduct.categories.some((cat: string) => product.categories!.includes(cat));
      ...
      <div className="max-w-7xl mx-auto px-4">
        <h2 className="text-2xl font-bold text-gray-800 mb-6">
          {isEnergySystemProduct ? (
            <span>
              ⚡ <span className="text-red-600">Sistema Energía Natural</span> - Productos Complementarios
            </span>
          ) : isAntiAgingSystemProduct ? (
            <span>
              🌿 <span className="text-green-600">Sistema Anti-Edad</span> - Productos Complementarios
            </span>
          ) : isArticularSystemProduct ? (
            <span>
              🏗️ <span className="text-blue-600">Sistema Articular Avanzado</span> - Productos Complementarios
            </span>
          ) : (
