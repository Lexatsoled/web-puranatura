...
import { loadSystems, loadFeaturedSystems, loadProductsBySystem } from '@/data/products';
...
export const SystemsTestPage: React.FC = () => {
  ...
  const [systemProducts, setSystemProducts] = useState<Record<string, Product[]>>({});
  ...
  useEffect(() => {
    Promise.all([
      loadSystems(),
      loadFeaturedSystems()
    ]).then(async ([allSystems, featured]) => {
      setSystems(allSystems);
      setFeaturedSystems(featured);
      
      // Cargar productos para cada sistema
      const productsMap: Record<string, Product[]> = {};
      for (const system of allSystems) {
        productsMap[system.id] = await loadProductsBySystem(system.id);
      }
                ...
                <div className="p-8">
                  ...
                  <div className="border-t pt-4">
                    ...
                    <div className="text-sm text-gray-600">
                      {systemProducts[system.id]?.length || 0} productos disponibles
                    </div>
                  </div>
                </div>
              </div>
