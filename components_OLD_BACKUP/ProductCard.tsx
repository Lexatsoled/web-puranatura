...
const ProductCard: React.FC<ProductCardProps> = memo(({ product }) => {
  ...
  return (
    <div className="bg-white rounded-lg shadow-md overflow-hidden group transform hover:-translate-y-1 transition-all duration-300 flex flex-col relative">
      {/* Badges */}
        ...
        <div className="w-full h-full flex items-center justify-center p-4">
          <picture>
            <source
              type="image/webp"
              srcSet={
                `${cardImageUrl.replace(/\.(jpg|jpeg)$/i, '_320.webp')} 320w, ` +
                `${cardImageUrl.replace(/\.(jpg|jpeg)$/i, '_640.webp')} 640w, ` +
                `${cardImageUrl.replace(/\.(jpg|jpeg)$/i, '_768.webp')} 768w, ` +
                `${cardImageUrl.replace(/\.(jpg|jpeg)$/i, '_1024.webp')} 1024w`
              }
          ...
          <div className="space-y-2">
            ...
            <div className="flex items-center justify-between">
              ...
              >
                {isOutOfStock
                  ? 'Agotado'
                  : `Añadir ${selectedQuantity > 1 ? `(${selectedQuantity})` : ''}`}
              </button>
