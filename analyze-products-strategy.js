...
const productTiers = {
  ...
  tier3: {
    ...
    description: "Productos herbales con evidencia mixta pero demanda comercial",
    products: [
...
function generateImplementationPlan() {
  ...
  Object.entries(productTiers).forEach(([tierKey, tier]) => {
    console.log(`\n${tier.name}`);
    ...
    tier.products.forEach((productId, index) => {
      ...
      else if (evidenceMap.established.includes(productId)) evidenceLevel = "✅ Establecida";
      else if (evidenceMap.mixed.includes(productId)) evidenceLevel = "⚖️ Mixta";
