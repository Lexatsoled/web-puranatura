...
- ✅ `http://localhost:3000/` → `{"error":"Ruta no encontrada","path":"/"}`  
  *(Respuesta correcta - error handler 404 funcionando)*
...
- [ ] `/api/auth/signup` crea usuarios *(validación pendiente - servidor no mantiene conexión desde scripts)*
- [ ] `/api/auth/login` autentica usuarios *(validación pendiente)*
- [ ] `/api/auth/me` retorna datos del usuario *(validación pendiente)*
