...
Estado actual: **Fase 5 en progreso** – el runbook `docs/runbooks/fase5-maintainability.md` y la ADR `docs/adr/0003-phase5-maintainability.md` definen cómo reducimos CC (T5.1), aplicamos Clean Architecture (T5.2), aseguramos pre-commit + lint/test obligatorios (T5.3) y actualizamos la documentación viva (T5.4) antes del próximo release. La evidencia solicita `reports/complexity-report.json`, `reports/synthetic/synthetic-report.json`, `reports/observability/observability-artifacts.zip` y `sbom.json` en cada PR.
