...
Estado actual: **Fase 4 completada** – Sprint 1 (T4.1/T4.2) cerrado con trazas+metrics (`backend/src/tracing/initTracing.ts`, `backend/src/middleware/traceId.ts`, `backend/src/utils/metrics.ts`, `docs/runbooks/observability.md`) y artefactos (`reports/observability/metrics-snapshot.txt`, `reports/observability/dashboard-summary.md`, `reports/observability/observability-artifacts.zip`). Sprint 2 (T4.3/T4.4) se ejecutó con el pipeline `ci-quality` (.github/workflows/ci-quality.yml, `npm run generate:sbom`, `sbom.json`, `reports/**`) y el runbook/canary `docs/runbooks/ci-canary.md` junto con `scripts/rollout-canary.cjs`, `scripts/update-flag.cjs` y `config/flags.json`. Sprint 3 (T4.5/T4.6) cerró la política de backup/DR (`GPT-51-Codex-Max-Hight/runbooks/backup-dr.md`) y los sintéticos (`scripts/synthetic-checks.ts`, `reports/synthetic/synthetic-report.json`, `reports/observability/dashboard-summary.md`) para enlazar alertas p95/p99/error-rate con el canary. Los artefactos (`reports/synthetic/synthetic-report.json`, `reports/observability/observability-artifacts.zip`, `sbom.json`) se preservan para cada release y auditoría de rollback.
Estado actual: **Fase 5 en progreso** – el runbook `docs/runbooks/fase5-maintainability.md`, el plan `GPT-51-Codex-Max-Hight/plan-maestro-fase-5.md` y la ADR `docs/adr/0003-phase5-maintainability.md` definen cómo reducimos CC (T5.1), aplicamos Clean Architecture (T5.2), aseguramos pre-commit + lint/test obligatorios (T5.3) y actualizamos la documentación viva (T5.4) antes del próximo release. La evidencia solicita `reports/complexity-report.json`, `reports/synthetic/synthetic-report.json`, `reports/observability/observability-artifacts.zip` y `sbom.json` en cada PR.
