...
- 2025-12-05: src/utils/contentSanitizers.product.ts agrega helpers sanitizeOptional, mapOptional y mapOrEmpty para centralizar la sanitización opcional, mantener components/faqs undefined cuando no existen y garantizar tags como array vacío; gates ejecutados: npm run lint, npm run test:ci, npm run check:complexity.
- 2025-12-06: src/utils/imageProcessor.helpers.ts introduce DEFAULT_OPTIONS, resizePipeline y formatProcessors para dejar el flujo de resize+formato en tres helpers reutilizables y eliminar el switch; gates: npm run lint, npm run test:ci, npm run check:complexity.
- 2025-12-06: operativa de `npx prettier --write` (src/pages/components/scripts/test) y ajuste de `scripts/check-secret-drift.cjs` (catch sin `err`) limpian los warnings de lint y dejan el gate verde; gates: npm run lint, npm run test:ci, npm run check:complexity.
- 2025-12-07: `src/utils/sanitizeObject.ts`, `src/components/virtualGrid/useGridLayout.ts` y `src/hooks/seo/buildSeoConfig.ts` estructuran helpers (`sanitizeArray`, `sanitizeRecord`, `resolveColumnCount`, `computeColumnWidth`, `buildOpenGraphImages`) para aplanar la lógica de sanitización, layout y SEO; gates: `npm run lint`, `npm run test:ci`, `npm run check:complexity`.
- 2025-12-07: `src/utils/sanitizeObject.ts` delega en `src/utils/sanitizeObject.helpers.ts`, `src/components/virtualGrid/useGridLayout.ts` en `src/components/virtualGrid/useGridLayout.helpers.ts` y `src/hooks/seo/buildSeoConfig.ts` en `src/hooks/seo/buildSeoConfig.helpers.ts`, manteniendo las funciones puras separadas para reducir la complejidad; gates: `npm run lint`, `npm run test:ci`, `npm run check:complexity`.
- 2025-12-08: `src/hooks/useAnalytics.ts` se apoya en `src/hooks/analytics/useAnalytics.helpers.ts` para separar la inicialización del consentimiento, la sincronización, el trackeo y la creación de eventos; gates: `npm run lint`, `npm run test:ci`, `npm run check:complexity`.
...
- 2025-12-11: `src/hooks/useProductDetails.ts` avanza el refactor al delegar fetch y fallback en `src/hooks/product/useProductDetails.helpers.ts`, donde `getFallbackProducts` cachea los datos legacy y `handleProductFetch` mantiene los estados `loading/ready/error`; el cambio se documenta en runbook/ADR y se comprueba con `npm run lint`, `npm run test:ci` y `npm run check:complexity`.
- 2025-12-12: `src/routes/dynamicRoutes.ts` se simplifica al delegar sus metadata generators en `src/routes/dynamicRoutes.helpers.ts` (helpers para categorías/productos/posts con `fetchJson`, badging de `structuredData` y generadores de paths); el ADR y el runbook mencionan el nuevo helper, y los tres gates siguen limpios (`npm run lint`, `npm run test:ci`, `npm run check:complexity`).
- 2025-12-13: `src/services/analyticsProviders.ts` introduce `src/services/analyticsProviders.helpers.ts` como una capa de helpers que arranca los scripts (`loadExternalScript`), asegura `dataLayer` y `fbq`, y encapsula `trackCustomEvent`; el ADR referencia este helper y recalca que no se rompieron los gates (`npm run lint`, `npm run test:ci`, `npm run check:complexity`).
- 2025-12-14: `src/utils/sanitizeObject.helpers.ts` maneja ahora `SANITIZER_RULES` y `findSanitizer`, permitiendo asociar ridículamente las claves `html/url` a sus sanitizadores sin lógicas condicionales pesadas; documentamos ese helper y confirmamos gates (`npm run lint`, `npm run test:ci`, `npm run check:complexity`).
- 2025-12-15: `src/hooks/useProductDetail.ts` se apoya en `src/hooks/product/useProductDetail.helpers.ts` para el keyboard nav y `handleAddToCart`, manteniendo el hook principal como un coordinador de estado simple; el ADR anota el helper y los gates (`npm run lint`, `npm run test:ci`, `npm run check:complexity`) siguen verdes.
- 2025-12-15: Se acepta mantener los módulos con CC 10-11 (utilidades de control como `api.ts`, `intl.ts` y middlewares) dado que el flujo condicional es inherente y el coste/beneficio de fraccionar más es bajo; la decisión queda registrada y los gates continúan verdes (`npm run lint`, `npm run test:ci`, `npm run check:complexity`).
- 2025-12-15: Cierre formal de Fase 5: gates finales (lint/test:ci/check:complexity) validados, módulos residuales en CC 10-11 aceptados y documentación sincronizada (runbook, checkpoints, checklist).
