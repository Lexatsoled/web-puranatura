...
const productMatches = productsFile.match(/{\s*id:\s*["']([^"']+)["'][^}]*?}/gs);
...
if (productMatches) {
  ...
  productMatches.forEach((match, index) => {
    ...
    if (idMatch && nameMatch) {
      ...
      const productEnd = nextProductStart > 0 ? nextProductStart : productsFile.length;
      const fullProduct = productsFile.substring(productStart, productEnd);
      
      // Verificar qué información falta
      const hasDetailedDescription = fullProduct.includes('detailedDescription:');
  ...
  productsWithoutComponents.forEach(p => console.log(`  - ${p.id}: ${p.name}`));
  ...
  productsWithoutQuestions.forEach(p => console.log(`  - ${p.id}: ${p.name}`));
  
  // Productos que tienen TODA la información faltante
  const completelyIncomplete = productsWithoutDetail.filter(p => 
