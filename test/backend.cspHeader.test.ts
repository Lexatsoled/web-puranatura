...
describe('CSP header behaviour', () => {
  it('expone header Content-Security-Policy-Report-Only en entorno report-only', async () => {
    ...
    const header = res.headers['content-security-policy-report-only'] || res.headers['content-security-policy'];
    expect(header).toBeDefined();
    expect(String(header)).toMatch(/script-src/);
    expect(String(header)).toMatch(/maps.googleapis.com|google-analytics.com|connect.facebook.net/);
