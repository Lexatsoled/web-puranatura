...
| Integration (`frontend-pages`, `frontend-core`) | Rutas + providers + servicios | Auth flow, StorePage filtros/paginación, Blog modal con DOMPurify, Wishlist + Cart | `vitest run --maxWorkers=1 --no-file-parallelism src/pages/**/*.test.tsx` |
...
| Seguridad | Escaneo secretos + dependencias + XSS | `gitleaks`, `npm audit`, payload XSS en blog/contacto | `gitleaks detect && npm audit --production` (en PowerShell usa `;` o ejecuta los comandos por separado) |
