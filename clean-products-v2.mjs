...
const endProductsIndex = lines.findIndex(line => line.includes('// ===== FIN DE PRODUCTOS ====='));
...
for (let i = firstArrayCloseIndex + 1; i < lines.length; i++) {
  if (lines[i].includes('export interface System {')) {
    ...
    if (lines[i+1] && lines[i+1].includes('id: string;') &&
        lines[i+2] && lines[i+2].includes('name: string;') &&
        lines[i+3] && lines[i+3].includes('description: string;')) {
...
if (firstArrayCloseIndex > 0 && correctInterfaceIndex > 0) {
  ...
  console.log(`Líneas a eliminar: ${correctInterfaceIndex - firstArrayCloseIndex - 1}`);
