...
for (let i = 1; i < productSections.length; i++) {
  ...
  if (idMatch) {
    ...
    const endOfProduct = section.indexOf('  },') !== -1 ? 
      section.substring(0, section.indexOf('  },') + 4) : section;
    
    // Verificar si tiene faqs
    const hasFaqs = endOfProduct.includes('faqs:');
    const hasUsage = endOfProduct.includes('usage:') || endOfProduct.includes('administrationMethod:');
    const hasComponents = endOfProduct.includes('components:');
    const hasDetailedDescription = endOfProduct.includes('detailedDescription:');
...
incompleteProducts.forEach(product => {
  console.log(`\n📦 ${product.id}`);
  console.log(`   ✅ Descripción detallada: ${product.detailedDescription ? 'SÍ' : '❌ NO'}`);
