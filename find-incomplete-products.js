...
// Mostrar resultados
console.log(`❌ Productos sin detailedDescription (${incomplete.noDetailedDescription.length}):`);
...
if (incomplete.noComponents.length <= 20) {
  ...
  incomplete.noComponents.slice(0, 20).forEach(id => console.log(`  - ${id}`));
...
if (incomplete.noFrecuentQuestions.length <= 20) {
  ...
  incomplete.noFrecuentQuestions.slice(0, 20).forEach(id => console.log(`  - ${id}`));
...
const completelyMissing = incomplete.noDetailedDescription.filter(id => 
...
console.log(`Sin detailedDescription: ${incomplete.noDetailedDescription.length}`);
