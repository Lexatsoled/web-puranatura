...
const ignoredDirs = ['node_modules', '.git', 'dist', 'coverage', '.lighthouseci', 'tmp', '.husky', 'test-results', 'playwright-report', 'reports'];
...
// Scan Modules
inventory.files = inventory.files.concat(scanDir(path.join(rootDir, 'src'), 'frontend-core'));
inventory.files = inventory.files.concat(scanDir(path.join(rootDir, 'backend'), 'backend-core'));
inventory.files = inventory.files.concat(scanDir(path.join(rootDir, 'scripts'), 'scripts'));
inventory.files = inventory.files.concat(scanDir(path.join(rootDir, 'e2e'), 'e2e'));
inventory.files = inventory.files.concat(scanDir(path.join(rootDir, 'docs'), 'docs'));
inventory.files = inventory.files.concat(scanDir(path.join(rootDir, 'infra'), 'infra'));
